            ...
            <TabsList className="grid w-full grid-cols-3 mb-6 gap-2">
              {providers.map((provider) => (
                <TabsTrigger
                  key={provider.id}
                  value={provider.id}
                  className=" data-[state=active]:bg-slate-100 data-[state=active]:text-white border-1 border-slate-200"
                >
                  <img
            ...
            {providers.map((provider) => (
              <TabsContent key={provider.id} value={provider.id}>
                <div className="flex flex-col">
                    ...
                    <AnimatePresence mode="wait">
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                          {providers[2].productTypes.map((subType) => (
                            <motion.div
                              key={subType.id}
                              initial={{ opacity: 0 }}
                              animate={{ opacity: 1 }}
                  ...
                  {currentProductType?.subTypes && currentProductType?.subTypes.length > 0 && (
                    <AnimatePresence mode="wait">
                      <motion.div
                        key={selectedType}
                        initial={{ opacity: 0, y: 20 }}
